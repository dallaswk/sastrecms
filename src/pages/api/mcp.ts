    ...
    switch (tool) {
      ...
      case "create_node": {
        ...
        await db.insert(nodes).values({
          id, siteId: SITE_ID, contentTypeId, parentId: parentId ?? null,
      ...
      case "update_settings": {
        ...
        if (row) {
          await db.update(settings).set(patch).where(eq(settings.siteId, SITE_ID));
        }
